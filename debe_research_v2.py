...
def make_research(search_web):
    def dynamic_research(model,engine,year,fuel):
        ...
        rows=[]
        # Sequential on purpose: Brave/search providers rate-limit concurrent cloud requests.
        for i,q in enumerate(queries):
            try: rows.extend(search_web(q,10))
            except Exception: pass
            if i==0 and len(base.dedup(rows,20))>=8: break
            if i==1 and len(base.dedup(rows,20))>=12: break
        rows=base.dedup(rows,22)

        pages={}
        with ThreadPoolExecutor(max_workers=6) as ex:
            jobs={ex.submit(read_page,r.get('url','')):r.get('url','') for r in rows[:9] if r.get('url')}
        ...
        for r in rows:
            ...
            if (model_ok and engine_ok) or engine_specific: docs.append((r,blob))
        ...
        return {'strengths':strengths[:3],'issues':outissues[:4],'checks':checks[:4],'sources':sources[:8],'evidence':evidence[:6],'research_score':max([x['confidence'] for x in evidence],default=30),'engine_focus':engine,'research_available':bool(evidence),'identity_used':identity,'search_results':len(rows),'relevant_sources':len(docs),'queries_used':queries}
