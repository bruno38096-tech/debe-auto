...
def search_web(q, n=10):
    try:
        r = requests.get('https://html.duckduckgo.com/html/?q=' + quote_plus(q), headers=HEADERS, timeout=(2, 4))
        r.raise_for_status()
        soup = BeautifulSoup(r.text, 'html.parser')
        out = []
...
def piscapisca_comparables(model,fuel,current_url='',limit=4):
    key=model_key(model); parts=key.split()
    if len(parts)<2: return []
    urls=[f'https://www.piscapisca.pt/carros/marca/{slugify(parts[0])}/{slugify(parts[1])}',f'https://www.piscapisca.pt/carros/{slugify(parts[0])}/{slugify(parts[1])}']
    soup=None
    for listing_url in urls:
        try:
            r=requests.get(listing_url,headers=HEADERS,timeout=(3,7)); r.raise_for_status(); soup=BeautifulSoup(r.text,'html.parser')
            if soup: break
        except Exception: continue
    if not soup: return []
    wanted=fuel_group(fuel); seen,out=set(),[]
    for a in soup.find_all('a',href=re.compile(r'/carros/usados/')):
        href=urljoin('https://www.piscapisca.pt',a.get('href',''))
        if not href or href in seen or (current_url and href.rstrip('/')==current_url.rstrip('/')): continue
        seen.add(href); node=a; block=clean(a.get_text(' ',strip=True))
        for _ in range(7):
            if node.parent is None: break
            node=node.parent; txt=clean(node.get_text(' ',strip=True))
            if 'km' in txt.lower() and ('€' in txt or 'EUR' in txt): block=txt; break
        ...
        if wanted and fuel_group(item_fuel) and fuel_group(item_fuel)!=wanted: continue
        year=y.group(1) if y else ''; km_val=numeric(km_m.group(1)) if km_m else 0; price_val=numeric(p_m.group(1)) if p_m else 0
        if not year or not km_val or not price_val: continue
        out.append({'title':re.sub(r'\s+\d[\d .]*\s*km.*$','',title).strip(),'year':year,'km':f'{km_val:,}'.replace(',','.')+' km','price':f'{price_val:,}'.replace(',','.')+' €','fuel':item_fuel or fuel,'score':vehicle_score(year,km_val),'url':href,'source':'PiscaPisca'})
...
def comparables():
    ...
    return jsonify(ok=True,match=model_key(model),fuel_group=fuel_group(fuel),deals=piscapisca_comparables(model,fuel,current_url))
...
def research():
    ...
    for issue in issues[:2]:
        ...
        elif any(x in low for x in ['battery','bateria','hybrid','híbrido']): title='Bateria / sistema eletrificado'; detail='Confirma autonomia, funcionamento do sistema híbrido, mensagens de erro e histórico de intervenções.'
        elif any(x in low for x in ['infotainment','sensor','electr']): title='Eletrónica'; detail='Testa sensores, câmaras, infotainment e assistências à condução.'
